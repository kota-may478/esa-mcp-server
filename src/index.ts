...
async function esaRequest(
  ...
  const url = `${ESA_API_BASE}/teams/${ESA_TEAM_NAME}${path}`;
  ...
  try {
    switch (name) {
      case "esa_list_posts": {
        ...
        const data = await esaRequest("GET", `/posts${query}`);
        return {
          content: [
            {
              type: "text",
      ...
      case "esa_get_post": {
        const data = await esaRequest("GET", `/posts/${args!.post_number}`);
        return {
      ...
      case "esa_create_post": {
        ...
        const data = await esaRequest("POST", "/posts", body);
      ...
      case "esa_update_post": {
        ...
        const data = await esaRequest("PATCH", `/posts/${postNumber}`, {
          post: postBody,
        });
        const post = data as { number: number; url: string; full_name: string };
        return {
      ...
      case "esa_delete_post": {
        const postNumber = args!.post_number;
        await esaRequest("DELETE", `/posts/${postNumber}`);
        return {
          content: [
            {
              type: "text",
              text: `✅ 記事 #${postNumber} を削除しました。`,
            },
          ],
        };
      }
      ...
      case "esa_get_members": {
        const data = await esaRequest("GET", "/members");
        return {
          content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
        };
      }
